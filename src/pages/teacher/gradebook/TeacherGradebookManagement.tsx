...
import { MagnifyingGlassIcon, Pencil2Icon, TrashIcon, EyeOpenIcon, PlusIcon, InfoCircledIcon } from '@radix-ui/react-icons';
import { useGradebookMutation } from '../../../hooks/useGradebookMutation';
import { Gradebook, Class, Subject } from '../../../types';
    ...
    <Card>
      <Flex direction="column" gap="3">
        ...
        <Grid columns="2" gap="3">
          <Box>
            <Badge color="green" size="2">A (90-100)</Badge>
            <Text size="2">Excellent</Text>
          </Box>
          <Box>
            <Badge color="blue" size="2">B (80-89)</Badge>
            <Text size="2">Good</Text>
          </Box>
          <Box>
            <Badge color="yellow" size="2">C (70-79)</Badge>
            <Text size="2">Satisfactory</Text>
          </Box>
          <Box>
            <Badge color="orange" size="2">D (60-69)</Badge>
            <Text size="2">Needs Improvement</Text>
          </Box>
          <Box>
            <Badge color="red" size="2">F (Below 60)</Badge>
        ...
        <Callout.Root color="blue">
          ...
          <Callout.Text>
            Grades are automatically calculated based on total marks, but can be manually adjusted if needed.
          </Callout.Text>
        </Callout.Root>
      </Flex>
    </Card>
...
const GradeBadge = ({ grade }: { grade?: string }) => {
  ...
  return (
    <Badge color={getGradeColor()} size="2">{grade}</Badge>
  );
};

const TeacherGradebookManagement = () => {
  ...
  const [publishedFilter, setPublishedFilter] = useState<string>('all_statuses');
  const [academicYearFilter, setAcademicYearFilter] = useState<string>('all_years');
  ...
  const teacherSubjects = useMemo(() => {
    if (!teacherClasses || !currentUser.data?._id) return [];

    const authorizedSubjects: any[] = [];
    const subjectMap = new Map<string, any>();

    // Loop through each class
    teacherClasses.forEach((classData: any) => {
  ...
  }, [teacherClasses, currentUser.data?._id]);

  // Remove the original teacher subjects query
  ...
  // Fetch gradebooks for the teacher's classes
  const { data: gradebooks, isLoading: isLoadingGradebooks } = useQuery<Gradebook[]>({
    ...
    queryFn: () => gradebookService.getTeacherGradebooks({
      class: classFilter !== 'all_classes' ? classFilter : undefined,
      subject: subjectFilter !== 'all_subjects' ? subjectFilter : undefined,
      term: termFilter !== 'all_terms' ? termFilter : undefined,
      isPublished: publishedFilter !== 'all_statuses' ? publishedFilter === 'published' : undefined,
      academicYear: academicYearFilter !== 'all_years' ? academicYearFilter : undefined
    }),
    enabled: !isLoadingClasses,
  });

  if (isLoadingGradebooks || isLoadingClasses) {
  ...
  const filteredGradebooks = gradebooks?.filter(gradebook => {
    ...
    // Grade filtering
    if (gradeFilter !== 'all_grades' && gradebook.finalGrade !== gradeFilter) {
      return false;
    }
  ...
  const getStudentName = (gradebook: Gradebook) => {
    return typeof gradebook.student === 'string' 
      ? gradebook.student 
      : `${gradebook.student.firstName} ${gradebook.student.lastName}`;
  };

  const getClassName = (gradebook: Gradebook) => {
    if (!teacherClasses) return 'Unknown Class';
    
    return typeof gradebook.class === 'string' 
      ? teacherClasses.find(c => c._id === gradebook.class)?.name || 'Unknown Class'
      : gradebook.class.name;
  };

  const getSubjectName = (gradebook: Gradebook) => {
    if (!teacherSubjects || teacherSubjects.length === 0) return 'Unknown Subject';
    
    return typeof gradebook.subject === 'string' 
      ? teacherSubjects.find(s => s._id === gradebook.subject)?.name || 'Unknown Subject'
    ...
    <Card size="4">
      <Flex direction="column" gap="5">
        <Flex justify="between" align="center">
          ...
          <Flex gap="2">
            <Button onClick={() => setShowGradingScale(!showGradingScale)} variant="soft">
              {showGradingScale ? 'Hide' : 'Show'} Grading Scale
            </Button>
            <Button asChild>
              <Link to="/teacher/gradebook/create">Add New Grades</Link>
            </Button>
          </Flex>
        </Flex>
          ...
          <Box mt="4">
            <Flex justify="between" align="center" wrap="wrap" gap="3">
              <Box style={{ minWidth: '250px', flexGrow: 1 }}>
                <TextField.Root placeholder="Search by student name..."
                    value={searchTerm}
              ...
              <Flex gap="3" wrap="wrap">
                ...
                >
                  ...
                  <Select.Content>
                    <Select.Item value="all_subjects">All Subjects</Select.Item>
                    {teacherSubjects && teacherSubjects.map((s) => (
                ...
                >
                  ...
                  <Select.Content>
                    <Select.Item value="all_terms">All Terms</Select.Item>
                ...
                >
                  ...
                  <Select.Content>
                    <Select.Item value="all_years">All Years</Select.Item>
                ...
                >
                  ...
                  <Select.Content>
                    <Select.Item value="all_grades">All Grades</Select.Item>
            ...
            <Tabs.Content value="published">
              {renderGradebooksTable(paginatedGradebooks.filter(g => g.isPublished))}
            </Tabs.Content>
      ...
      return (
        <Card mt="4">
          <Flex direction="column" align="center" justify="center" p="6" gap="4">
            <Text color="gray" size="3">No gradebook entries found</Text>
            <Button asChild>
              <Link to="/teacher/gradebook/create">
                <PlusIcon /> Add New Grades
              </Link>
            </Button>
          </Flex>
        </Card>
              ...
              <Table.Cell>
                <Flex gap="2">
                  <Button asChild size="1" variant="soft">
                    <Link to={`/teacher/gradebook/${gradebook._id}`}>
                      <EyeOpenIcon />
                    </Link>
                  </Button>
                  <Button asChild size="1" variant="soft">
                    <Link to={`/teacher/gradebook/${gradebook._id}/edit`}>
                      <Pencil2Icon />
                    </Link>
                  </Button>
...
export default TeacherGradebookManagement; 